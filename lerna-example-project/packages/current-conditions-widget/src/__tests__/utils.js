...
test('mostly clear description should be shown if it\'s "Mostly Sunny" and it is after sunset', () => {
  const isNightIconShouldBeShown = true;
  const weatherCode = 5; // Mostly Sunny code
  const clearOrMostlyClearDescription = getClearOrMostlyClearDescription(
    isNightIconShouldBeShown,
    weatherCode,
  );

  expect(clearOrMostlyClearDescription).toEqual("mostlyClear");
});
