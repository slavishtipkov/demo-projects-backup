...
export interface Props {
  ...
  readonly setIsSameStationSelected: (isStationSame: boolean) => SetIsSameStationSelected;
}
...
export default class IndexView extends React.Component<Props> {
  ...
  render(): JSX.Element {
    const selectedOptionStation = this.props.stations
      .slice(0, 3)
      .find(s => s.stationId === this.props.selectedStation.stationId);
        ...
        <I18nConsumer>
          {({ t }) => (
            <div data-testid="location-select-view">
              ...
              <DropdownWrapper innerRef={(comp: any) => (this.dropdownRef = comp)}>
                ...
                <DropdownContainer
                  expanded={this.props.isDropdownExpanded && this.props.stations.length > 0}
                >
                  {this.props.stations.slice(0, 3).map((station: Station, index: number) => (
                    <DropdownItem
  ...
  private readonly changeSelectedStation = (station: Station) => {
    this.props.setIsSameStationSelected(this.props.selectedStation.stationId === station.stationId);
